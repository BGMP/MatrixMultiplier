...
data_long <- data %>%
  pivot_longer(cols = c(Traditional, DR1, DR2), names_to = "Algorithm", values_to = "Time")

plot_traditional <- ggplot(data, aes(x = n, y = Traditional, color = "Traditional")) + 
  ...
  scale_color_manual(values = c("Traditional" = "blue"), labels = "Tradicional") +    
  theme_classic() +     
  theme(
...
plot_loglog_square_ticks <- ggplot(data_long, aes(x = n, y = Time, color = Algorithm)) +
  geom_point(size = 2.5) +          
  geom_line() +                     
  scale_x_continuous(trans = "log10", 
                     breaks = c(32, 64, 128, 256, 512, 1024), 
...
print(plot_loglog_square_ticks)